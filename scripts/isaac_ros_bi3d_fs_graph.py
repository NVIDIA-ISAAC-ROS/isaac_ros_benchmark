...
"""
Performance test for the Isaac Bi3D Freespace graph.
...
- Graph under Test:
    ...
    2. FreespaceSegmentationNode: creates occupancy grid from proximity segmentation disparity image
...
def launch_setup(container_prefix, container_sigterm_timeout):
    """Generate launch description for benchmarking Isaac ROS freespace segmentation graph."""

    bi3d_node = ComposableNode(
        name='Bi3DNode',
        namespace=TestIsaacROSBi3DFreespaceGraph.generate_namespace(),
    ...
    monitor_node = ComposableNode(
        ...
        remappings=[
            ('output', 'freespace_segmentation/occupancy_grid')],
    )
...
def generate_test_description():
    MODELS_ROOT = os.path.join(TestIsaacROSBi3DFreespaceGraph.get_assets_root_path(), 'models')
    FEATNET_MODEL_FILE_PATH = os.path.join(MODELS_ROOT, FEATNET_MODEL_FILE_NAME)
    ...
    return TestIsaacROSBi3DFreespaceGraph.generate_test_description_with_nsys(launch_setup)


class TestIsaacROSBi3DFreespaceGraph(ROS2BenchmarkTest):
    """Performance test for the Isaac Bi3D Freespace graph."""

    # Custom configurations
    config = ROS2BenchmarkConfig(
        benchmark_name='Isaac ROS Bi3D Freespace Graph Benchmark',
